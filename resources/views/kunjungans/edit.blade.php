                    ...
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-white">
                            <div class="d-flex justify-content-between align-items-center">
                                <h5 class="card-title mb-0">Edit Kunjungan {{ $kunjungan->pasien->name }}</h5>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <form action="{{ route('kunjungans.update', $kunjungan->id) }}" method="POST">
                                ...
                                <input type="hidden" class="form-control id" name="pasien_id" value="{{ $kunjungan->pasien->id }}">
                                <input type="hidden" class="form-control id" name="user_id" value="{{ $kunjungan->user_id }}">
                                
                                <div class="form-group mb-3">
                                    <label for="nik" class="form-label">NIK</label>
                                    <input type="text" class="form-control nik" id="nik" name="nik" placeholder="NIK" value="{{ $kunjungan->pasien->nik }}">
                                    @error('nik')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control name" id="name" name="name" 
                                        placeholder="Nama" value="{{ $kunjungan->pasien->name }}">
                                    @error('name')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control alamat" id="alamat" name="alamat"
                                        placeholder="Alamat" value="{{ $kunjungan->pasien->alamat }}">
                                    @error('alamat')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control kecamatan" id="kecamatan" name="kecamatan"
                                            placeholder="Kecamatan" value="{{ $kunjungan->pasien->district_id }}">
                                    </div>
                                    <div class="col-md-6">
                                        <label for="kabupaten" class="form-label">Kabupaten/Kota</label>
                                        <input type="text" class="form-control kabupaten" id="kabupaten" name="kabupaten"
                                            placeholder="Kabupaten/Kota" value="{{ $kunjungan->pasien->regency_id }}">
                                    </div>
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="date" id="inputDay" class="form-control" name="tanggal" 
                                        placeholder="Tanggal" value="{{ $kunjungan->tanggal }}">
                                </div>
                                
                                <div class="form-group mb-3">
                                    <label for="status" class="form-label">Status</label>
                                    <select name="status" id="status" class="form-select">
                                        <option value="Belum" {{ $kunjungan->pasien->status == 'Belum' ? 'selected' : '' }}>Belum</option>
                                        <option value="Sudah" {{ $kunjungan->pasien->status == 'Sudah' ? 'selected' : '' }}>Sudah</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <select name="jenis" id="jenis" class="form-select">
                                        <option value="Rencana" {{ $kunjungan->jenis == 'Rencana' ? 'selected' : '' }}>Rencana</option>
                                        <option value="Lanjutan" {{ $kunjungan->jenis == 'Lanjutan' ? 'selected' : '' }}>Lanjutan</option>
                                    </select>
                                </div>
...
@push('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check if there's a date already selected
            const inputDay = document.getElementById("inputDay");
            if (inputDay.value) {
                const dateParts = inputDay.value.split('-');
                if (dateParts.length === 3) {
                    const selectedYear = parseInt(dateParts[0]);
                    const selectedMonth = parseInt(dateParts[1]) - 1;
                    ...
                    // Highlight the day if it's in current view
                    setTimeout(() => {
                        const days = document.querySelectorAll('.day');
                        days.forEach(day => {
                            const dayNum = day.querySelector('strong');
                            if (dayNum && parseInt(dayNum.textContent) === selectedDay) {
