...
class UserImport implements ToCollection, WithHeadingRow, WithChunkReading, WithBatchInserts
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            //\Log::debug((array)$row);
            $nama_puskesmas_pembantu = $row['nama_puskesmas_pembantu'];
            $kelurahan = $row['kelurahan'];
            $nama_perawat_koordinator = $row['nama_perawat_koordinator'];
            $email = $row['email'];
            ...
            // cek ke tabel pustu, jika belum ada insert
            $village = \DB::table('villages')->where('name', $kelurahan)->first();
            $pustu = \App\Models\Pustu::firstOrCreate(
                ['nama_pustu' =>  $nama_puskesmas_pembantu ?? '-'],
                [
            ...
            // jika belum ada user, create baru
            if ($email != null) {
                $pustu = \App\Models\Pustu::where('nama_pustu', $nama_puskesmas_pembantu)->first();
                \App\Models\User::firstOrCreate(
                    ['email' => $email],
                    [
                        'name' => $nama_perawat_koordinator,
