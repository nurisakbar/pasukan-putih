...
class PasienWilayahReportController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        foreach ($groupedData as $wilayahKey => $wilayahData) {
            foreach ($wilayahData['pasien'] as $pasien) {
                $flatData->push([
                    ...
                    'diagnosis_penyakit' => $pasien['diagnosis_penyakit'] ?? null,
                    'nama_penginput' => $pasien['nama_penginput'] ?? '-',
                ]);
            }
        }
        ...
        return DataTables::of($sortedData)
            ...
            ->addColumn('diagnosis_penyakit', function ($row) {
                return $row['diagnosis_penyakit'] ?? '-';
            })
            ->addColumn('nama_penginput', function ($row) {
                return $row['nama_penginput'] ?? '-';
            })
            ->make(true);
    }
